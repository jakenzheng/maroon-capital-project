...
Examples:
  ...
  python run_live.py --symbol PM,MNST --strategy mystrategy --dry-run --live
    ...
    parser.add_argument("--lookback", type=int, default=1000, help="Bars to fetch each iteration (default: 200)")
    ...
    parser.add_argument("--position-size", type=float, default=100000.0, help="Per-trade position size (default: 10.0)")
    ...
    # Pairs params (only used for MyStrategy)
...
def main() -> None:
    ...
    if is_pairs(args.symbol):
        ...
        # Force MyStrategy in pairs mode (you can loosen this later)
        strategy = MyStrategy(
        ...
        def handle_iteration() -> None:
            ...
            iteration_count += 1
            logger.debug(f"Iteration {iteration_count}: fetching data for {symbol_a},{symbol_b}")
            
            net_a = trader_a._get_net_position()
            net_b = trader_b._get_net_position()
            if net_a == 0 and net_b == 0:
                equity = float(trader_a.api.get_account().equity)
                strategy.position_size = 0.10 * equity
                
            df = trader_a.run_once_pairs(trader_b)
    ...
    if strategy_cls is MovingAverageStrategy:
        strategy = MovingAverageStrategy(short_window=args.short_window, long_window=args.long_window, position_size=args.position_size)
        ...
        strategy = CryptoTrendStrategy(short_window=args.short_window, long_window=args.long_window, position_size=args.position_size)
    elif strategy_cls is DemoStrategy:
        strategy = DemoStrategy(position_size=args.position_size)
    else:
        try:
            strategy = strategy_cls()
        except TypeError as exc:
            raise SystemExit(f"{strategy_cls.__name__} must support a no-arg constructor or use --strategy template.") from exc
